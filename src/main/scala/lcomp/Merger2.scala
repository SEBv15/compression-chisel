...
/** Wrapper for Merge2 and MergeWeird2
 *
 ...
 */
class Merger2(val wordsize:Int = 16, val inwords1:Int = 10, val inwords2:Int = 10, val minwords1:Int = 0, val minwords2:Int = 0, val merge_weird:Boolean = true) extends Module {
    ...
    if (merge_weird) {
        val weird_merger = Module(new MergeWeird2(wordsize, inwords1, inwords2, minwords1, minwords2))
        io <> weird_merger.io
    } else {
        val merger = Module(new Merge2(wordsize, inwords1, inwords2, minwords1))
        io <> merger.io        
    }
}

object Merger2 extends App {
    chisel3.Driver.execute(args, () => new Merger2)
}
