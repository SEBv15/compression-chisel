...
class PatternEncoder extends Module {
    ...
    enc := (encw(14) === 2.U && io.in(14) === 0.U) || (encw(14) === 1.U && io.in(14) === 1.U) || io.in === 0.U

    // Calculate the position and length
    val posw = Wire(Vec(15, UInt(4.W)))
    val lenw = Wire(Vec(15, UInt(4.W)))
    val notdonew = Wire(Vec(15, Bool())) // Indicates whether the run has stopped (maybe not needed because of above code)
    posw(0) := 0.U
    lenw(0) := io.in(0)
    notdonew(0) := 1.B
    for (i <- 1 until 15) {
        when (io.in(i)) {
            notdonew(i) := notdonew(i-1)
            when (lenw(i-1) === 0.U) {
                posw(i) := i.U
                lenw(i) := 1.U
            }.otherwise {
                posw(i) := posw(i-1)
                lenw(i) := lenw(i-1) + notdonew(i)
            }
        }.otherwise {
            lenw(i) := lenw(i-1)
            posw(i) := posw(i-1)
